...
CREATE TABLE IF NOT EXISTS `Leave` (
  ...
  FOREIGN KEY (`Applicant_ID`) REFERENCES `Student` (`USN`)
    ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB;
...
-- ─── USER_PASSWORD TABLE ───────────────────────────────────
-- A normalized version using separate role table OR a unique table per role is better.
-- MySQL does NOT support multiple FK references from one column to different tables.
-- Recommended: use a `role` to redirect to appropriate table in application logic.
...
FOR EACH ROW
BEGIN
  IF NEW.Start_Date < CURDATE() + INTERVAL 1 DAY THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Start_Date must be at least tomorrow';
  END IF;
END$$
